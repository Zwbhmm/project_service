new Vue({
    ...
    methods:{
        findByCid:function(current_cid,currentPage){
            ...
            var _this = this;
            axios.get('/project_service/pro/findByCid/' + current_cid + "?currentPage=" + currentPage)
        ...
        getProsByName:function (current_pName,currentPage) {
            //在当前方法中定义一个变量，表明是vue对象
            var _this = this;
            axios.get('/project_service/pro/getProByName/' + current_pName + "?currentPage=" + currentPage)
                .then(function (response) {
                    _this.proList = response.data;
                })
                .catch(function (error) {
        ...
        getTotalPageByCid:function(current_cid){
            //在当前方法中定义一个变量，表明是vue对象
            var _this = this;
            axios.get('/project_service/pro/getTotalPageByCid/' + current_cid)
                .then(function (response) {
        ...
        getTotalPageByName:function(current_pName){
            //在当前方法中定义一个变量，表明是vue对象
            var _this = this;
            axios.get('/project_service/pro/getTotalPageByName/' + current_pName)
                .then(function (response) {
                    _this.pro.totalPage = response.data;
                })
                .catch(function (error) {
        ...
        getLoginUser:function () {
            //在当前方法中定义一个变量，表明是vue对象
            var _this = this;
            axios.get('/project_service/shopUser/getUser')
                .then(function (response) {
        ...
        loginOut:function () {
            if (confirm("是否确定要退出账号？")) {
                axios.get('/project_service/shopUser/loginOut')
                    .then(function (response) {
                        //把返回的user对象的id赋值给这里的id
                        if (response.data == "OK") {
                            alert("退出成功！");
