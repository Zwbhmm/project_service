...
import com.project.service.shopService.ShopUserService;
import com.project.util.shopUtils.Md5Utils;
import com.project.util.shopUtils.TimeUtils;
...
public class ShopUserController {
     ...
     */
    @GetMapping("/login")
    public ShopUser login(ShopUser user, HttpServletRequest request){
        ...
        //根据用户名和密码寻找到User对象
        ShopUser loginUser = userService.login(user);
     ...
     */
    @GetMapping("/findAll")
    public List<ShopUser> findAll(@RequestParam(value = "currentPage", defaultValue = "1") Integer currentPage,
                              @RequestParam(value = "pageSize", defaultValue = "10") Integer pageSize){
        //使用分页工具进行分页操作
        PageHelper.startPage(currentPage,pageSize);

        List<ShopUser> userList = userService.findAll();
    ...
    public String register(@RequestBody ShopUser user){
        //设置注册的时间为当前的时间
     ...
     */
    @GetMapping("/getUser")
    public ShopUser getUser(HttpServletRequest request){
    ...
    public String updateUserByUsername(ShopUser user, HttpServletRequest request){
        //System.out.println(user);
        //初始化session
        HttpSession session = request.getSession();
        //先获取旧密码，如果旧密码跟新密码一样就不用改
        ShopUser old_user = (ShopUser) session.getAttribute("user");
    ...
    public ShopUser getUserById(@PathVariable Integer userid, HttpServletRequest request){
    ...
    public String updateUserById(@PathVariable Integer userid,
        ...
        ShopUser current_user = (ShopUser) session.getAttribute("current_user");
